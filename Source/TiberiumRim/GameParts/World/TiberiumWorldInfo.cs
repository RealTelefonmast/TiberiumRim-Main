...
public class TiberiumWorldInfo : WorldInformation
{
    ...
    public override void Notify_EventHappened(string tag, IIncidentTarget target)
    {
        if (tag == "TiberiumArrival")
        {
            SpawnTiberiumTile(target.Tile);
        }
    }
        
    private void SpawnTiberiumTile(int tile)
    {
